...
export const AssetsWidget = memo(({ assets = [], setCurrentView, ...wProps }) => {
  ...
  return (
    <DashboardWidget
      ...
      icon={<Icon name="devices_other" className="text-foreground shrink-0" size={22}/>}
      {...wProps}
      action={
        <button onClick={() => setCurrentView('assets')} className="apple-glass-btn text-xs font-semibold px-3.5 h-7 rounded-full text-foreground hover:text-foreground/80 cursor-pointer">
          Manage
        </button>
      }
    >
