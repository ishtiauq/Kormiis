...
export default function AiCoPilotModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] pointer-events-none flex items-start justify-end p-3 sm:p-5 pt-[80px] sm:pt-[92px] pr-3 sm:pr-6 bg-transparent animate-in fade-in duration-200">
      <div 
        ref={modalRef}
        className="pointer-events-auto w-[calc(100vw-2rem)] sm:w-[440px] md:w-[460px] h-[min(76vh,580px)] flex flex-col rounded-[24px] glass-kormiis border border-white/35 dark:border-white/16 shadow-[0_24px_55px_-12px_rgba(0,0,0,0.50),0_0_20px_0_rgba(0,0,0,0.20)] overflow-hidden backdrop-blur-3xl relative animate-in slide-in-from-top-4 zoom-in-95 duration-200"
      >
        {/* MODAL HEADER */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/60 dark:border-white/10 shrink-0 bg-white/20 dark:bg-white/[0.03]">
          <div className="flex items-center gap-2.5">
            <div className="size-8 rounded-xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 flex items-center justify-center shadow-xs shrink-0">
              <Icon name={showHistoryDrawer ? "history" : "auto_awesome"} size={17} className="text-white dark:text-neutral-900" />
            </div>
            <div>
          ...
          {/* Close/Back button in History View */}
          {showHistoryDrawer && (
            <button
              onClick={() => setShowHistoryDrawer(false)}
              aria-label="Back to chat"
              className="apple-glass-btn size-7.5 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground cursor-pointer transition-all active:scale-95"
        ...
        {showHistoryDrawer ? (
          ...
          <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in duration-200 relative">
            ...
            <div className="p-2.5 sm:p-3 border-t border-border/60 dark:border-white/10 bg-white/30 dark:bg-white/[0.04] shrink-0 space-y-2">
