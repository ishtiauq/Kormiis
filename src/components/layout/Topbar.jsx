      ...
      >
        ...
        <div className="sm:hidden absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-auto">
          <a
            ...
            className="flex items-center justify-center cursor-pointer outline-none no-underline border-none bg-transparent p-0 m-0 shadow-none select-none transition-transform active:scale-95"
            style={{ background: 'transparent', border: 'none', boxShadow: 'none', padding: 0, margin: 0 }}
            title="Kormiis Dashboard"
          >
            <img
        ...
        <div className="flex items-center gap-[1.6px] shrink-0 h-[48px] sm:h-[54px] ml-auto sm:ml-0">
          ...
          {/* Notifications Trigger (Desktop & iPad / Tablet - hidden only on mobile phone handsets) */}
          ...
          {/* User Profile Trigger Button (Containerless - Hidden on mobile screens since it's present in the mobile bottom bar) */}
          {user && (
            <div className="hidden sm:flex items-center justify-center">
      ...
      {showNotifications && (
        ...
        >
          ...
          <div className="flex flex-col max-h-[min(65vh,540px)] overflow-y-auto p-4 sm:p-5">
            ...
            <div className="flex items-center gap-1.5 p-1 bg-muted/50 dark:bg-white/[0.06] rounded-2xl border border-border/60 dark:border-white/[0.08] mb-3 shrink-0">
              <button 
                ...
                filteredNotifications.map(n => {
                  ...
                  return (
                    <div 
                      ...
                      className={`group p-3 rounded-2xl transition-all duration-200 cursor-pointer border relative select-none flex items-start gap-3 active:scale-[0.99] ${
                        isUnread 
                    ...
                    >
                      <Icon name={meta.icon} size={20} className="shrink-0 text-foreground transition-transform group-hover:scale-105 mt-0.5" />

                      <div className="min-w-0 flex-1 flex flex-col gap-1">
                        {/* Top row: Headline aligned with icon on left, NEW & time on right */}
                        <div className="flex items-start justify-between gap-2">
                          <h4 className="text-fluid-sm font-bold text-foreground leading-snug break-words">
                            {headline}
                          </h4>
                        ...
                        {/* Body details text */}
                        {body && (
                          <p className={`text-fluid-xs m-0 leading-relaxed text-foreground/85 dark:text-foreground/90 break-words ${isUnread ? 'font-medium' : 'font-normal text-muted-foreground'}`}>
                            {body}
                          </p>
                        )}
                      </div>
