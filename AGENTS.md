...
# Code Modification & Output Standard (Micro-diffs / Surgical Edits Only)

> **CRITICAL INSTRUCTION**: Ekhon theke puro code rewrite na kore shudhu matro proyojonio poriborton-tuku (Micro-diffs / surgical edits) output dibe ebong file edit korar shomoy exact target lines edit korbe.

1. **No Full-file Rewrites**:
   - Never regenerate, overwrite, or rewrite an entire file when applying fixes, improvements, or feature additions.
   - Do NOT dump full source code blocks in chat or responses.
2. **Micro-diffs / Surgical Changes Only**:
   - Only touch and output the minimal, precise block of code required for the change.
   - When modifying files, target only the exact lines being updated (using focused replacements / surgical chunks).
   - In conversation responses, only show the concise diff or relevant snippet showing what changed, never the whole file.
